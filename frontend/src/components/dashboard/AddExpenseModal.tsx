    ...
    <AnimatePresence>
      <motion.div
        ...
        <motion.div
          ...
          style={{ background:'var(--surface)',border:'1px solid var(--glass-border)',borderRadius:'24px',padding:'28px',width:'100%',maxWidth:'480px',boxShadow:'var(--shadow)',backdropFilter:'blur(24px)',WebkitBackdropFilter:'blur(24px)' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
            {/* Type toggle */}
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'6px',background:'var(--glass)',padding:'4px',borderRadius:'12px',border:'1px solid var(--glass-border)' }}>
              {(['expense','income'] as const).map(t => (
                <button key={t} type="button" onClick={() => setType(t)}
                  style={{
                    padding:'9px', borderRadius:'9px', border:'none', cursor:'pointer',
                    fontFamily:'inherit', fontSize:'13.5px', fontWeight:500, transition:'all 0.15s',
            ...
            <div>
              ...
              <input type="number" value={amount} onChange={e=>setAmount(e.target.value)}
                ...
                onFocus={e=>{e.currentTarget.style.borderColor='var(--accent)';e.currentTarget.style.background='var(--glass-hover)';}}
            ...
            <motion.button
              ...
              style={{ background:'var(--accent)',color:'#fff',border:'none',borderRadius:'50px',padding:'13px',fontSize:'14.5px',fontWeight:700,cursor:loading?'not-allowed':'pointer',opacity:loading?0.7:1,fontFamily:'inherit',boxShadow:'0 4px 20px var(--accent-dim)',transition:'filter 0.15s' }}>
